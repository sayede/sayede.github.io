...
* Credit = Henrik MÃ¼he (modified by Adlane SAYEDE)
*
* Issues:
...
function BibtexParser() {
    ...
    this.errorThrown = function(str) {
        jQuery_old("#bibtex_errors").html(str);
    }
    ...
    this.entry_body = function(directive) {
        this.currentEntry = this.key(); //l'id BIBTEXKEY
        ...
        if (directive == "@INCOLLECTION") {
            ...
            this.entries[this.currentEntry]["BIBTEXTYPE"] = "poster";
        } else if (directive == "@BOOK") {
            ITYPE = "BOOK";
            this.entries[this.currentEntry]["BIBTEXTYPE"] = "book";
...
function BibtexDisplay() {
    ...
    this.createTemplate = function(entry) {
        // find template
        var tpl = jQuery_old(".bibtex_template").clone().removeClass('bibtex_template');
        ...
        tpl.find('.bibtexVar').each(function() {
            ...
            jQuery_old.each(this.attributes, function(i, attrib) {
        ...
        var groups=["CMNM","CSMN","MOEN","OXID","RM2I","CATEN","MATCAT","MODSPE","REMCAT","VALBIO","CASECO","CASU","CISCO","MCP"]
		var UCCS = JSON.parse('{\"CASU\":[{\"url\":\"www.uccs.univ-artois.fr\/equaxe1.htm\"},{\"firstName\":\"BLETA\"},{\"firstName\":\"PONCHEL\"},{\"firstName\":\"BRICOUT\"},{\"firstName\":\"FERREIRA\"},{\"firstName\":\"HAPIOT\"},{\"firstName\":\"LÉGER\"},{\"firstName\":\"MACHUT\"},{\"firstName\":\"ROUSSEAU\"},{\"firstName\":\"TILLOY\"},{\"firstName\":\"WYRWALSKI\"},{\"firstName\":\"MONFLIER\"}],' +
            '\"CMNM\":[{\"url\":\"www.uccs.univ-artois.fr\/equaxe2.htm\"},{\"firstName\":\"BLACH\"},{\"firstName\":\"CHAMBRIER\"},{\"firstName\":\"FERRI\"},{\"firstName\":\"HENNINOT\"},{\"firstName\":\"MEZIANE\"},{\"firstName\":\"SAITZEK\"},{\"firstName\":\"SAYEDE\"},{\"firstName\":\"DESFEUX\"}]}');
        ...
        for (var index in keys) {
            ...
            if (key == "BIBTEXRAW") {
                tpl.find("." + key.toLowerCase()).html(value);
	            	...
	            	if (value == "conference, workshop") {
	                    newvalue = "<a style=\"text-decoration: none\"><span class=\"label label-comm\">COMM</label></a>"
	                    tpl.find(".ITYPE").html(newvalue);
	                    tpl.addClass("Conf");
	                } else if (value == "book chapter") {
	                    newvalue = "<a style=\"text-decoration: none\"><span class=\"label label-couv\">BOOK</label></a>"
	                    tpl.find(".ITYPE").html(newvalue);
	                    tpl.addClass("Book");
	                }else if (value == "poster") {
	                    newvalue = "<a style=\"text-decoration: none\"><span class=\"label label-poster\">POSTER</label></a>"
	                    tpl.find(".ITYPE").html(newvalue);
	                    tpl.addClass("Poster");
	                }else if (value == "book") {
	                    newvalue = "<a style=\"text-decoration: none\"><span class=\"label label-couv\">BOOK</label></a>"
	                    ...
	                    newvalue = "<a style=\"text-decoration: none\"><span class=\"label label-art\">ARTICLE</label></a>"
	                    tpl.find(".ITYPE").html(newvalue);
	                    tpl.addClass("Article");
	                }
            	}
            	else if (key == "AUTHOR") {
				...
				$.each(UCCS, function(j, gname) {
					$.each(gname, function(i, pname) {
						if (pname.firstName){
		                    if (~value.toLowerCase().indexOf(pname.firstName.toLowerCase())) {
		                        group = " <a  href=\"http://"+gname[0].url+"\" target=\"_blank\" style=\"text-decoration: none\"><span class=\"label label-group\">"+j+"</label></a>"
		                        tpl.find(".group").append(group);
		                        tpl.addClass(j);
		                        return false;
		                    }
						}    
                ...
                value = "<a href=\"http://dx.doi.org/" + value + "\" target=\"_blank\" style=\"text-decoration: none\"><span class=\"label label-doi\">DOI</label></a>"
                tpl.find("." + key.toLowerCase()).html(value);
            } else if (key == "URL") {
                value = "<a href=\"" + value + "\" target=\"_blank\" style=\"text-decoration: none\"><span class=\"label label-url\">HAL</label></a>"
                tpl.find("." + key.toLowerCase()).html(value);
            } else {
    ...
    this.createStructure = function(structure, output, entries, level) {
        ...
        if (groupChild.length) {
            ...
            var values = [];
            jQuery_old.each(sortedArray, function(i, object) {
            ...
            var topics = jQuery_old(".bibtex_topics");

            // Iterate through the values and recurively call this function
            globalStruct = jQuery_old('<div></div>');
            for (val in values) {
    ...
    this.displayBibtex = function(input, output) {
        ...
        var structure = jQuery_old(".bibtex_structure").clone();
        ...
        if (structure.length) {
            ...
            jQuery_old(".bibtex_structure").remove();
        } else {
...
function bibtex_js_draw(newbib) {
    jQuery_old(".bibtex_wait").hide();
    (new BibtexDisplay()).displayBibtex(newbib, jQuery_old("#bibtex_display"));
    CMNM = $('.CMNM').length; //CMNM=CMNM-1; if (CMNM<0){CMNM=0};
    CASU = $('.CASU').length; //CASU=CASU-1; if (CASU<0){CASU=0};
    Articles = $('.Article').length; //Articles=Articles-1; if (Articles<0){Articles=0};
    Books = $('.Book').length; //Books=Books-1; if (Books<0){Books=0};
    Conf = $('.Conf').length; // Conf = Conf - 1; if (Conf < 0) {Conf = 0};
    Poster = $('.Poster').length; // Conf = Conf - 1; if (Conf < 0) {Conf = 0};
    $('#CMNMl').text(CMNM)
    $('#CASUl').text(CASU)
    $('#Articlel').text(Articles)
    $('#Bookl').text(Books)
    $('#Confl').text(Conf);
    $('#Posterl').text(Poster);
}
...
function BibTeXSearcher() {
    ...
    this.checkEntry = function(entry, word) {
        ...
        if (word[0] != "@") {
            entry.find("span:not(.noread)").each(
                function() {
                    if (jQuery_old(this).text().search(new RegExp(word, "i")) > -1 &&
                        entry.is(":visible")) {
                        found = true;
            ...
            if (arrayStr.length < 2) {
                entry.find("span:not(.noread)." + arrayStr[0]).each(
                    function() {
                        if (jQuery_old(this).text().search(new RegExp(strings[1], "i")) > -1 &&
                            entry.is(":visible")) {
                            found = true;
                            return false; //Break out of loop
                        }
                ...
                switch (arrayStr[1]) {
                    case "first":
                        entry.find("span:not(.noread)." + arrayStr[0]).each(
                            function() {
                                arrayString = jQuery_old(this).text().split(new RegExp(",[\\s]+and[\\s]+|,[\\s]+"));
    ...
    this.unhideAll = function() {
        jQuery_old("div#bibtex_display").children().each(
            function() {
                jQuery_old(this).show();
                jQuery_old(this).find(".bibtexentry").each(
                    function() {
                        jQuery_old(this).show();
                    });
            });
    ...
    this.hideEntry = function(word) {
        var funcCaller = this;
        var container = jQuery_old("div#bibtex_display").children();
        if (container.first().hasClass("bibtexentry:visible")) {
            container.each(
                function() {
                    if (!funcCaller.checkEntry(jQuery_old(this), word)) {
                        jQuery_old(this).hide();
                    }
                });
        } else {
            container.each(
                function() {
                    var shouldHide = true;
                    jQuery_old(this).find(".bibtexentry:visible").each(
                        function() {
                            if (!funcCaller.checkEntry(jQuery_old(this), word)) {
                                jQuery_old(this).hide();
                            } else {
...
function createWebPage(defaultTemplate) {
    ...
    jQuery_old(document).ready(function() {
        // check for template, add default
        //if (jQuery_old(".bibtex_template").lenght == 0) {
        //  jQuery_old("body").append(defaultTemplate);
        //}
        //bibtex_js_draw();
    });
}
...
function loadExtras() {
    ...
    jQuery_old(".bibtex_author").each(function(i, obj) {
        authorList(jQuery_old(this));
    });
    ...
    jQuery_old("select").each(function() {
        if (localStorage.getItem(jQuery_old(this).attr("id"))) {
            jQuery_old(this).val(JSON.parse(localStorage.getItem(jQuery_old(this).attr("id"))));
        }
    });

    jQuery_old(".bibtex_search").each(function(i, obj) {
        jQuery_old(this).on('change', function(e) {
            combineSearcher(BibTeXSearcherClass, true);
            localStorage.setItem(jQuery_old(this).attr("id"), JSON.stringify(jQuery_old(this).val()));
        });
        jQuery_old(this).keyup(function() {
            combineSearcher(BibTeXSearcherClass);
        });
        if (jQuery_old(this).val() != "") {
            combineSearcher(BibTeXSearcherClass, true);
        }
    });
...
function combineSearcher(searcherClass, needToRestart) {
    needToRestart = typeof needToRestart !== 'undefined' ? needToRestart : false;
    var string = "";
    jQuery_old("select.bibtex_search").each(function(i, obj) {
        var front = "";
        if (obj.hasAttribute("search"))
            front = "@" + jQuery_old(this).attr("search");
        if (obj.hasAttribute("extra")) {
            front += "[" + jQuery_old(this).attr("extra") + "]=";
        } else {
            if (front != "") {
                front += "=";
            }
        ...
        if (jQuery_old(this).val() != "") {
            string += "%" + front + jQuery_old(this).val();
        }
    });
    jQuery_old("input.bibtex_search").each(function(i, obj) {
        if (jQuery_old(this).val() != "") {
            string += "%" + jQuery_old(this).val().split(' ').join('%');
        }
    });
...
function authorList(object) {
    var map = new Object();
    jQuery_old("span.author").each(function(i, obj) {
        arrayString = jQuery_old(this).text().split(new RegExp(",[\\s]+and[\\s]+|,[\\s]+"));
    ...
    for (var i = 0; i < tuples.length; i++) {
        ...
        object.append(jQuery_old("<option></option>").attr("value", key).text(text));
    }
}
